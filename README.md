...
[![CI](https://github.com/aryankasaudhan07/mailtrace/actions/workflows/ci.yml/badge.svg)](https://github.com/aryankasaudhan07/mailtrace/actions/workflows/ci.yml)
...
**One command** — it uses Docker if available, otherwise runs everything locally:
...
| Open | URL |
|---|---|
| Web app (React UI) | http://localhost:8080 *(Docker)* · http://localhost:5173 *(local)* |
| API docs (Swagger) | http://localhost:8000/docs |
| Live campaign graph | http://localhost:8000/live |
...
```bash
make start   # Docker stack if available, else local — auto-detects (UI :8080 or :5173)
make dev     # run API (:8000) + React UI (:5173) locally, no Docker
make stop    # stop the Docker stack
make setup   # install backend (venv) + frontend (npm) dependencies
make test    # backend tests + lint
make build   # production build of the frontend
make help    # list every target
```

First clone? Run `make setup` once to install dependencies. The stack runs in
**fixture mode** (in-memory SQLite) — no Postgres, Redis, network, or API keys
required. Optional keys in `.env` unlock live extras — `GEMINI_API_KEY` for M4's
NLP content analysis and `ABUSEIPDB_KEY` for live IP reputation; without them the
app falls back to offline heuristics.

### What you get

A full-stack platform: **seven analyzers** (M2–M7 + trust boundary) producing
explainable `Evidence`, one weight-table scorer, a live campaign-correlation
graph, and a **React dashboard** (light/dark) covering Login, Dashboard, Analyze,
Result, Forensic, History, Reports, Threat Intelligence, Geolocation, Settings
and Help.

### Deploy to the cloud

One Docker image serves the API and UI from a single origin. See **`DEPLOY.md`**
for the Render blueprint (`render.yaml` + `Dockerfile.render`) — push to GitHub,
create a Blueprint, done. The same image runs on Railway, Fly.io, Cloud Run, or
any VPS.
...
pytest                                                 # 40 pass, 5 skipped (the skips are Track A's spec)
...
That output is the whole architecture working: analyzers emit `Evidence`, the
scorer turns it into one explainable verdict. All seven analyzers are now
implemented, so a real phishing sample scores CRITICAL with a full breakdown —
see the **Result** and **Forensic** views in the web app for the same data
rendered for an analyst.
...
tests/                  26 passing; 6 skipped ones are Track A's definition of done
```
